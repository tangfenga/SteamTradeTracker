...
class MongoDB(object):
    def __init__(self, collection, database="steam"):
        self.client = pymongo.MongoClient(host="localhost", port=int(MONGODB_PORT))
...
class TaskList(object):
    def __init__(self):
        self.redis = redis.Redis(host="localhost", port=int(REDIS_PORT), db=0, decode_responses=True)
